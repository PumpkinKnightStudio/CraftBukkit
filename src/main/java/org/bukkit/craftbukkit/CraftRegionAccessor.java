...
public abstract class CraftRegionAccessor implements RegionAccessor {
    ...
    public Biome getBiome(int x, int y, int z) {
        return CraftBiome.biomeBaseToBiome(getHandle().registryAccess().registryOrThrow(IRegistry.BIOME_REGISTRY), getHandle().getNoiseBiome(x >> 2, y >> 2, z >> 2));
    }
    ...
    public void setBiome(int x, int y, int z, Biome biome) {
        ...
        BiomeBase biomeBase = CraftBiome.biomeToBiomeBase(getHandle().registryAccess().registryOrThrow(IRegistry.BIOME_REGISTRY), biome);
