...
public class CraftMetaCrossbow extends CraftMetaItem implements CrossbowMeta {
    ...
    public void addChargedProjectile(ItemStack item) {
        ...
        Preconditions.checkArgument(item.getType() == ItemType.FIREWORK_ROCKET || CraftItemType.bukkitToMinecraft(item.getType()) instanceof ItemArrow, "Item %s is not an arrow or firework rocket", item);
